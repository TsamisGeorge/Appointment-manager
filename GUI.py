...
class Appointment_manager(Appointment_methods, Customers_methods,):#serch methods ως ορισμα
    def __init__(self):
        #     MAIN WINDOW    #
        ######################
...
#################################
        ...
        self.time_picker = ttk.Combobox(self.appointments_tab, values=[f"{str(h).zfill(2)}:{str(m).zfill(2)}" for h in range(9, 24) for m in range(0, 60, 10)])
...
################προσθήκη Treeview#############################################
        ...
        notification_button=tk.Button(self.search_tab,text="Send Email")
        notification_button.place(x=380,y=250)
